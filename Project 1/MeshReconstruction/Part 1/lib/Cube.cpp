...
Vec3 Cube::LerpVertex(double isoLevel, int i1, int i2) const {
    ...
    Vec3 const &p1 = pos[i1];
    Vec3 const &p2 = pos[i2];
...
Cube::Cube(Rect3 const &space, Fun3s const &sdf, FunTr const &transform, double twist) {
    double mx = space.min.x;
    double my = space.min.y;
    double mz = space.min.z;

    double sx = space.size.x;
    double sy = space.size.y;
    double sz = space.size.z;
    ...
    for (int i = 0; i < 8; ++i) {
        double sd = transform(sdf, pos[i], twist);
        if (sd == 0) sd += 1e-6;
        this->sdf[i] = sd;
    }
}
...
IntersectInfo Cube::Intersect(double iso) const {
    ...
    for (int e = 0; e < 12; ++e) {
        if (signConfigToIntersectedEdges[intersect.signConfig] & edges[e].edgeFlag) {
            ...
            Vec3 vert = LerpVertex(iso, v0, v1);
